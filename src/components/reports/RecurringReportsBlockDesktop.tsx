...
export const RecurringReportsBlockDesktop: React.FC = () => {
  const { user } = useAuth();
  const { data: recTransactions, isLoading } = useQuery({
    queryKey: ['recurring-reports-desktop', user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
      let query = supabase
        .from('transactions')
  ...
  return (
    <Card className="mt-6">
      ...
      <CardContent className="space-y-6">
        {incomeRec.length > 0 && (
          <div>
            <div className="font-semibold text-sm text-green-700 mb-2">Receitas Recorrentes</div>
            {incomeRec.map((t: any) => (
              <div key={t.id} className="border rounded-lg p-4 mb-3 bg-green-50/20">
                <div className="flex items-center justify-between mb-1">
                  <span className="font-medium text-base flex items-center gap-2">
                    <Repeat className="w-5 h-5 text-green-600" /> {t.title}
                  </span>
                  <Badge className="bg-green-100 text-green-700 border-green-200">{recurrenceLabel(t)}</Badge>
                </div>
                <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
                  <TrendingUp className="w-4 h-4 text-green-600" /> {formatCurrency(t.amount)}
                  {t.categories && (
                    <span className="ml-2 flex items-center gap-1"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: t.categories.color }} /> {t.categories.name}</span>
                  )}
                </div>
                {t.next_recurrence_date && (
                  <div className="flex items-center gap-1 text-xs text-gray-500">
                    <Calendar className="w-4 h-4" /> Próxima: {formatDate(t.next_recurrence_date)}
                  </div>
                )}
              </div>
            ))}
        ...
        {expenseRec.length > 0 && (
          <div>
            <div className="font-semibold text-sm text-red-700 mb-2">Despesas Recorrentes</div>
            {expenseRec.map((t: any) => (
              <div key={t.id} className="border rounded-lg p-4 mb-3 bg-red-50/20">
                <div className="flex items-center justify-between mb-1">
                  <span className="font-medium text-base flex items-center gap-2">
                    <Repeat className="w-5 h-5 text-red-600" /> {t.title}
                  </span>
                  <Badge className="bg-red-100 text-red-700 border-red-200">{recurrenceLabel(t)}</Badge>
                </div>
                <div className="flex items-center gap-2 text-sm text-muted-foreground mb-1">
                  <TrendingDown className="w-4 h-4 text-red-600" /> {formatCurrency(t.amount)}
                  {t.categories && (
                    <span className="ml-2 flex items-center gap-1"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: t.categories.color }} /> {t.categories.name}</span>
                  )}
                </div>
