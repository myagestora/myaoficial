...
export const AlertsInsightsBlock: React.FC<AlertsInsightsBlockProps> = ({ reportData, goalsData, transactions }) => {
  React.useEffect(() => {
    ...
    if (reportData && Array.isArray(reportData.categoryData)) {
      console.log('categoryData (desktop):', reportData.categoryData);
    }
  }, [reportData]);
  // Alertas
  ...
  if (goalsData && goalsData.length > 0) {
    goalsData.forEach(goal => {
      if (goal.status === 'exceeded') {
        alerts.push({
          icon: <AlertTriangle className="w-5 h-5 text-red-600" />, text: `Meta excedida: ${goal.title}`, color: 'text-red-700'
        });
      }
      if (goal.status !== 'exceeded' && goal.usagePercentage && goal.usagePercentage >= 90) {
        alerts.push({
          icon: <AlertTriangle className="w-5 h-5 text-orange-600" />, text: `Meta quase excedida: ${goal.title}`, color: 'text-orange-700'
        });
      }
      if (goal.status !== 'completed' && goal.progressPercentage && goal.progressPercentage >= 90) {
        insights.push({
          icon: <CheckCircle className="w-5 h-5 text-green-600" />, text: `Meta próxima de ser atingida: ${goal.title}`, color: 'text-green-700'
        });
      }
    });
  }
  ...
  if (reportData && reportData.balance < 0) {
    alerts.push({
      icon: <AlertTriangle className="w-5 h-5 text-red-600" />, text: 'Seu saldo está negativo!', color: 'text-red-700'
    });
  }
  // Saldo muito baixo
  if (reportData && reportData.balance >= 0 && reportData.balance < 100) {
    alerts.push({
      icon: <AlertTriangle className="w-5 h-5 text-orange-600" />, text: 'Seu saldo está muito baixo!', color: 'text-orange-700'
    });
  }
  ...
  if (transactions && transactions.length > 0) {
    ...
    transactions.forEach(t => {
      if (t.is_recurring && t.next_recurrence_date) {
        ...
        if (next < now) {
          alerts.push({
            icon: <AlertTriangle className="w-5 h-5 text-orange-600" />, text: `Recorrência atrasada: ${t.title}`, color: 'text-orange-700'
          });
        }
      }
    });
  ...
  if (transactions && transactions.length > 0) {
    ...
    Object.entries(catSums).forEach(([cat, values]) => {
      ...
      if (max > avg * 2 && max > 100) {
        alerts.push({
          icon: <AlertTriangle className="w-5 h-5 text-orange-600" />, text: `Despesa incomum em ${cat}: R$ ${max.toFixed(2)}`, color: 'text-orange-700'
        });
      }
    });
  }
  ...
  if (transactions && transactions.length > 0) {
    ...
    Object.entries(catSums).forEach(([cat, values]) => {
      ...
      if (max > avg * 2 && max > 100) {
        insights.push({
          icon: <TrendingUp className="w-5 h-5 text-green-600" />, text: `Receita incomum em ${cat}: R$ ${max.toFixed(2)}`, color: 'text-green-700'
        });
      }
    });
  }
  ...
  if (transactions && transactions.length > 0) {
    ...
    Object.entries(dayCounts).forEach(([d, count]) => {
      if (count >= 10) {
        alerts.push({
          icon: <AlertTriangle className="w-5 h-5 text-orange-600" />, text: `Muitas transações em ${d}: ${count}`, color: 'text-orange-700'
        });
      }
    });
  }
  ...
  if (reportData && reportData.monthlyData && reportData.monthlyData.length > 2) {
    ...
    if (last.expenses > prev.expenses) {
      insights.push({
        icon: <TrendingUp className="w-5 h-5 text-red-600" />, text: 'Despesas aumentaram em relação ao mês anterior', color: 'text-red-700'
      });
      if (reportData.categoryData && reportData.categoryData.length > 0) {
        ...
        insights.push({
          icon: <TrendingUp className="w-5 h-5 text-red-600" />, text: `Maior crescimento de despesas: ${topCat.name}`, color: 'text-red-700'
        });
      }
    } else if (last.expenses < prev.expenses) {
      insights.push({
        icon: <TrendingDown className="w-5 h-5 text-green-600" />, text: 'Despesas diminuíram em relação ao mês anterior', color: 'text-green-700'
      });
    }
  }
  ...
  if (reportData && Array.isArray(reportData.categoryData) && reportData.categoryData.length > 0) {
    ...
    if (topCat && topCat.name) {
      insights.push({
        icon: <Info className="w-5 h-5 text-blue-600" />, text: `Categoria mais usada: ${topCat.name}`, color: 'text-blue-700'
      });
    }
  }
  // Dica de economia
  if (reportData && reportData.totalExpenses > 0 && reportData.totalIncome > 0 && reportData.totalExpenses > 0.8 * reportData.totalIncome) {
    insights.push({
      icon: <Info className="w-5 h-5 text-purple-600" />, text: 'Suas despesas estão acima de 80% das receitas. Reveja seus gastos!', color: 'text-purple-700'
    });
  }
    ...
    <Card className="mt-6">
      ...
      <CardContent className="space-y-4">
        {alerts.length === 0 && insights.length === 0 && (
          <div className="text-sm text-gray-400">Nenhum alerta ou insight relevante para o período.</div>
        )}
        {alerts.length > 0 && (
          <div>
            <div className="font-semibold text-sm text-red-700 mb-2">Alertas</div>
            {alerts.map((a, i) => (
              <div key={i} className={`flex items-center gap-2 py-2 px-3 rounded ${a.color} bg-red-50/40 mb-2`}>
                {a.icon}
        ...
        {insights.length > 0 && (
          <div>
            <div className="font-semibold text-sm text-blue-700 mb-2 mt-2">Insights</div>
            {insights.map((a, i) => (
              <div key={i} className={`flex items-center gap-2 py-2 px-3 rounded ${a.color} bg-blue-50/40 mb-2`}>
                {a.icon}
                <span className="text-sm flex-1 truncate">{a.text}</span>
              </div>
            ))}
