...
export const GoalsSection: React.FC<GoalsSectionProps> = ({ dateRange }) => {
  const { user } = useAuth();
  ...
  const { data: goalsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Filtrar metas relevantes para o período
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-4 text-muted-foreground">
            Nenhuma meta ativa encontrada para o período selecionado.
          </div>
        </CardContent>
      </Card>
    );
