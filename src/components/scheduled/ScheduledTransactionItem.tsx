...
export const ScheduledTransactionItem = ({
  ...
  return <div className="rounded-lg border hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
      <div className="flex items-center justify-between p-4">
        <div className="flex items-center space-x-4">
          <div className={`p-3 rounded-full relative ${transaction.type === 'income' ? 'bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-400' : 'bg-red-100 text-red-600 dark:bg-red-900 dark:text-red-400'}`}>
            ...
            {(transaction.is_recurring || transaction.parent_transaction_id) && <Repeat className="h-3 w-3 absolute -top-1 -right-1 bg-blue-500 text-white rounded-full p-0.5" />}
          </div>
          <div>
            ...
            <div className="flex items-center space-x-3 mt-1">
              ...
              {transaction.recurrence_frequency && <Badge variant="outline" className="text-xs">
            ...
            {transaction.recurrence_count && <p className="text-xs text-gray-500 mt-1">
                {transaction.recurrence_count} repetições
              </p>}
          </div>
        </div>
        <div className="flex items-center space-x-4">
          <span className={`font-bold text-xl ${transaction.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>
            {transaction.type === 'income' ? '+' : ''}R$ {Math.abs(Number(transaction.amount)).toLocaleString('pt-BR', {
            minimumFractionDigits: 2
          })}
          </span>
          <div className="flex space-x-2">
            <Button variant="ghost" size="sm" title="Editar agendamento" onClick={() => onEdit?.(transaction.id)}>
              <Edit className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700" onClick={() => onDelete(transaction.id)} title="Excluir agendamento">
      ...
      {deletingId === transaction.id && onConfirmDelete && onCancelDelete && <div className="border-t p-4">
          <InlineConfirmation message="Confirmar exclusão?" onConfirm={onConfirmDelete} onCancel={onCancelDelete} confirmText="Excluir" cancelText="Cancelar" />
        </div>}
      
      {recurringDeletingId === transaction.id && onDeleteSingle && onDeleteSeries && onCancelDelete && <div className="border-t p-4">
          <RecurringDeletionOptions onDeleteSingle={onDeleteSingle} onDeleteSeries={onDeleteSeries} onCancel={onCancelDelete} isParent={transaction.is_recurring} />
        </div>}
    </div>;
};